...
############-------------Mouse E12: re-analysis on Vladoiu's scRNA-seq Data

##The single-cell RNA sequencing (scRNA-seq) dataset for mouse developmental time point embryonic day 12 (E12, N=9) has been taken from:
...
#Farshid Ghiyamihoor
#Analysis done Between March 2nd to April 5th, 2025
...
setwd("C:/Users/ghiyamif/OneDrive - University of Manitoba/eCN_project/Datasets/Vladoiu/E12")

# Load the sparse matrix
expression_matrix <- readMM("GSM3318000_E12_matrix.mtx")

# Load gene names (features)
features <- read.table("C:GSM3318000_E12_genes.tsv", 
...
barcode_folder <- "GSM3318000_E12_barcodes/"
...
dim(expression_matrix)  # Should be genes x cells. It returned [1] 27998  7540
length(barcodes)        # Should match the number of columns in expression_matrix: [1] 7540
...
#Save the Seurat Object (recommended).
saveRDS(seurat_object, file = "S_Object_Vladoiu_E12_raw_FG1.rds")
...
seurat_object <- readRDS("S_Object_Vladoiu_E12_raw_FG1.rds")
...
#####################################################################################################################################
############-------------Step 5: Filter Low-Quality Cells:filter cells based on mitochondrial percentage, gene count, and RNA count.

#For example: Filtering to set a condition to
...
saveRDS(seurat_object, file = "S_Object_Vladoiu_E12_filtered_FG2.rds")

#####################################################################################################################################
############-------------Step 6: Normalize the data

#You can work on the saved filtered data in step 5 (which is a seurat object)
seurat_object <- readRDS("S_Object_Vladoiu_E12_filtered_FG2.rds")

seurat_object <- NormalizeData(seurat_object, normalization.method = "LogNormalize")

saveRDS(seurat_object, file = "S_Object_Vladoiu_E12_normalized_FG3.rds")
...
seurat_object <- readRDS("S_Object_Vladoiu_E12_normalized_FG3.rds")
...
eCN_pure <- subset(eCN_candidates,  
                   (Meis2 > 0.5 | Lhx9 > 0.5 | Tbr1 > 0.5 | Olig2 > 0.5
                    | Lmx1a > 0.5 | Nhlh2 > 0.5 | Sox11 > 0.5 | Evx1 > 0.5) &  
                     Eomes == 0 & Neurod1 == 0 & Pax6 == 0)
#specific markers for eCN or expressed in subset of eCN: Meis2, Lhx9, Tbr1, Olig2, Lmx1a, Nhlh2, Sox11, Evx1
...
dim(eCN_pure)  # Returned: [1] 17212  3001

#Save subsetted  eCN_pure as E12_eCN
saveRDS(seurat_object, file = "S_Object_Vladoiu_E12_eCN_FG4.rds")

#####################################################################################################################################
############-------------Step 8: Find variable features and Scale the data

#You can work on the saved normalized data (which is a seurat object)
eCN_pure <- readRDS("S_Object_Vladoiu_E12_eCN_FG4.rds")
...
#####################################################################################################################################
############-------------Step 9: Run PCA and tSNE (or UMAP)

eCN_pure <- RunPCA(eCN_pure, assay = "RNA")

#####################################################################################################################################
############-------------Step 10: Run tSNE (or UMAP) (one of them is needed for paper)
...
####-----------To check number of suitable cluster -------------------------------------
#It gives many clusters
#I try tweaking resolution to hit ~15 clusters (previously published data in the Nature)
eCN_pure <- FindClusters(eCN_pure, resolution = seq(0.05, 0.2, by = 0.01))
#Then I use clustree to visualize how the clusters evolve:

clustree(eCN_pure)  #this is a clustree plot, and it’s super helpful for interpreting how clustering resolution affects your cell type assignments. 
####------------------------------------------------------------------------------------
...
saveRDS(eCN_pure, file = "S_Object_Vladoiu_E12_eCN_clustered_FG5.rds")
# Read the clustered seurat object
eCN_pure <- readRDS("S_Object_Vladoiu_E12_eCN_clustered_FG5.rds")
...
top_20_markers <- cluster_markers %>% group_by(cluster) %>% top_n(n = 20, wt = avg_log2FC)
...
write.xlsx(sheets5, file = "E12_markers5.xlsx")
write.xlsx(sheets10, file = "E12_markers10.xlsx")
write.xlsx(sheets20, file = "E12_markers20.xlsx")
write.xlsx(sheets50, file = "E12_markers50.xlsx")
