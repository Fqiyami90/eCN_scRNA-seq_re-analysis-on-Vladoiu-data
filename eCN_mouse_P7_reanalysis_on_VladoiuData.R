...
###############-----------------Mouse P7: re-analysis on Vladoiu's scRNA-seq Data-------------------------------#################

##The single-cell RNA sequencing (scRNA-seq) dataset for mouse developmental time point postnatal day 7 (P7, N=1) has been taken from:
...
setwd("C:/Users/ghiyamif/OneDrive - University of Manitoba/eCN_project/Datasets/Vladoiu/P7")

# Load the sparse matrix
expression_matrix <- readMM("GSM3318006_P7_matrix.mtx")

# Load gene names (features)
features <- read.table("GSM3318006_P7_genes.tsv", 
...
barcode_folder <- "GSM3318006_P7_barcodes/"
...
dim(expression_matrix)  # Should be genes x cells: [1] 27998  7179
length(barcodes)        # Should match the number of columns in expression_matrix: [1] 7179
...
saveRDS(seurat_object, file = "S_Object_Vladoiu_P7_raw_FG1.rds")
...
seurat_object <- readRDS("S_Object_Vladoiu_P7_raw_FG1.rds")
...
#Save the seurat object
saveRDS(seurat_object, file = "S_Object_Vladoiu_P7_filtered_FG2.rds")
...
seurat_object <- readRDS("S_Object_Vladoiu_P7_filtered_FG2.rds")

seurat_object <- NormalizeData(seurat_object, normalization.method = "LogNormalize")

saveRDS(seurat_object, file = "S_Object_Vladoiu_P7_normalized_FG3.rds")
...
seurat_object <- readRDS("S_Object_Vladoiu_P7_normalized_FG3.rds")
...
dim(eCN_pure)  # Returned: [1] 16964   896

#Save subsetted  eCN_pure as P7_eCN
saveRDS(seurat_object, file = "S_Object_Vladoiu_P7_eCN_FG4.rds")

#####################################################################################################################################
############-------------Step 8: Find variable features and Scale the data

#You can work on the saved normalized data (which is a seurat object)
eCN_pure <- readRDS("S_Object_Vladoiu_P7_eCN_FG4.rds")
...
saveRDS(eCN_pure, file = "S_Object_Vladoiu_P7_eCN_clustered_FG5.rds")
# Read the clustered seurat object
eCN_pure <- readRDS("S_Object_Vladoiu_P7_eCN_clustered_FG5.rds")
...
#Annotate the Clusters to generate annotated tSNE

new.cluster.ids <- c("eCN1", "eCN2", "eCN3", "eCN4", "eCN5", "eCN6", "eCN7", "eCN8", "eCN9", "eCN10", "eCN11", "eCN12", "eCN13", "eCN14", "eCN15")
...
write.xlsx(sheets5, file = "P7_markers5.xlsx")
write.xlsx(sheets10, file = "P7_markers10.xlsx")
write.xlsx(sheets20, file = "P7_markers20.xlsx")
write.xlsx(sheets50, file = "P7_markers50.xlsx")
