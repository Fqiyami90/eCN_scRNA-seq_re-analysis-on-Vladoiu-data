...
###############------------Mouse P14: re-analysis on Vladoiu's scRNA-seq Data-------------------------#####################

##The single-cell RNA sequencing (scRNA-seq) dataset for mouse developmental time point postnata day 14 (P14, N=1) has been taken from:
...
setwd("C:/Users/ghiyamif/OneDrive - University of Manitoba/eCN_project/Datasets/Vladoiu/P14")

# Load the sparse matrix
expression_matrix <- readMM("GSM3318007_P14_matrix.mtx")

# Load gene names (features)
features <- read.table("GSM3318007_P14_genes.tsv", 
...
barcode_folder <- "GSM3318007_P14_barcodes/"
...
dim(expression_matrix)  # Should be genes x cells: [1] 27998  4515
length(barcodes)        # Should match the number of columns in expression_matrix: [1] 4515
...
saveRDS(seurat_object, file = "S_Object_Vladoiu_P14_raw_FG1.rds")
...
seurat_object <- readRDS("S_Object_Vladoiu_P14_raw_FG1.rds")
...
#Save the seurat object
saveRDS(seurat_object, file = "S_Object_Vladoiu_P14_filtered_FG2.rds")
...
seurat_object <- readRDS("S_Object_Vladoiu_P14_filtered_FG2.rds")

seurat_object <- NormalizeData(seurat_object, normalization.method = "LogNormalize")

saveRDS(seurat_object, file = "S_Object_Vladoiu_P14_normalized_FG3.rds")
...
seurat_object <- readRDS("S_Object_Vladoiu_P14_normalized_FG3.rds")
...
dim(eCN_pure)  # Returned: [1] 17212  1358

#Save subsetted  eCN_pure as P14_eCN
saveRDS(seurat_object, file = "S_Object_Vladoiu_P14_eCN_FG4.rds")

#####################################################################################################################################
############-------------Step 8: Find variable features and Scale the data

#You can work on the saved normalized data (which is a seurat object)
eCN_pure <- readRDS("S_Object_Vladoiu_P14_eCN_FG4.rds")
...
eCN_pure <- FindClusters(eCN_pure, resolution = 0.25)  # Custom resolution - Higher resolution (> 0.8) → More clusters - finer splitting
...
saveRDS(eCN_pure, file = "S_Object_Vladoiu_P14_eCN_clustered_FG5.rds")
# Read the clustered seurat object
eCN_pure <- readRDS("S_Object_Vladoiu_P14_eCN_clustered_FG5.rds")
...
#Annotate the Clusters to generate annotated umap
#Based on the marker genes you find for each cluster, you can annotate the clusters with meaningful names 
#that reflect their identity. For example, if a cluster highly expresses known markers for a specific cell type, 
#you can annotate that cluster with that cell type's name.
#To annotate the clusters, you can add metadata to your Seurat object.

new.cluster.ids <- c("eCN1", "eCN2", "eCN3", "eCN4", "eCN5", "eCN6", "eCN7", "eCN8", "eCN9", "eCN10", "eCN11", "eCN12", "eCN13", "eCN14", "eCN15", "eCN16", "eCN17")
...
write.xlsx(sheets5, file = "P14_markers5.xlsx")
write.xlsx(sheets10, file = "P14_markers10.xlsx")
write.xlsx(sheets20, file = "P14_markers20.xlsx")
write.xlsx(sheets50, file = "P14_markers50.xlsx")

#####################################################################################################################################
############-------------Step 15: Plot marker genes or DEG

#To create plot
# Select top 5 or 10 markers for each cluster
